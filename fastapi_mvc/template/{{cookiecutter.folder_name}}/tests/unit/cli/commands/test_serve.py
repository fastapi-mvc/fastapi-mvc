...
@pytest.mark.parametrize("opts, expected", [
    (
        ...
        {"host": "localhost", "port": "5000", "workers": "2", "daemon": False, "env": (), "pid": None, "config": None}
    ),
    (
        ["--host", "localhost", "-p", 5000, "-w", 2, "--daemon", "--env", "FOO=BAR", "--env", "USE_FORCE=True", "--pid", pid_file, "--config", conf_file],
        {"host": "localhost", "port": "5000", "workers": "2", "daemon": True, "env": ("FOO=BAR", "USE_FORCE=True"), "pid": pid_file, "config": conf_file}
    ),
    (
        ["--host", "localhost", "-p", 5000, "-w", 2, "-D", "-e", "FOO=BAR", "-e", "USE_FORCE=True", "--pid", pid_file, "-c", conf_file],
        {"host": "localhost", "port": "5000", "workers": "2", "daemon": True, "env": ("FOO=BAR", "USE_FORCE=True"), "pid": pid_file, "config": conf_file}
    )
])
